...
// @access  Public
export const getRandomPuzzle = async (req: Request, res: Response): Promise<void> => {
  try {
...
// @access  Public
export const getRandomPuzzleRated = async (req: Request, res: Response): Promise<void> => {
  const rating = parseInt(req.params.rating, 10);

  try {
    const count = await Puzzle.countDocuments({ 'Rating': { $gt: rating } });
    const randomIndex = Math.floor(Math.random() * count);
    const puzzle = await Puzzle.findOne({ 'Rating': { $gt: rating } }).skip(randomIndex);
    if (puzzle) {
      res.json(puzzle);
    } else {
      res.status(404).json({ message: 'Puzzle not found' });
...
// @access  Public
export const getNextPuzzle = async (req: Request, res: Response): Promise<void> => {
